 ...
 */
namespace {
#define TIMER_SNAKE_TICK 1
bool sSnakeVisible = false;

SnakePage* snakePage() {
	return static_cast<SnakePage*>(
		PageManager::getInstance().getPage("SnakePage"));
}

void resetSnakeTimer() {
	SnakePage* page = snakePage();
	if (page) {
		mActivityPtr->resetUserTimer(TIMER_SNAKE_TICK, page->getTickIntervalMs());
	}
}
...
static void onUI_show() {
	...
	PageManager::getInstance().navigateTo("SnakePage");
	mActivityPtr->registerUserTimer(TIMER_SNAKE_TICK, 350);
	resetSnakeTimer();
}
...
static void onUI_hide() {
	...
	mActivityPtr->unregisterUserTimer(TIMER_SNAKE_TICK);
}
 ...
 *            触摸事件将继续传递到控件上
 */
static bool ongameSnakeActivityTouchEvent(const MotionEvent &ev) {
